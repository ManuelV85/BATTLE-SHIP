...
export const boardColums = 10;
export const BOARD = boardColums * boardRows;
...
export const stateToClass = {
  ...
  [squareState.ship_sunk]: 'ship-sunk',
...
export const generateEmptyLayout = () => {
  return new Array(boardRows * boardColums).fill(squareState.empty);
};
...
export const isWithinBounds = (entity) => {
  return (
    ...
    (entity.orientation === 'horizontal' &&
      entity.position.x + entity.length <= boardColums)
  );
};
...
export const putEntityInLayout = (oldLayout, entity, type) => {
  ...
  if (type === 'ship-sunk') {
    entityIndices(entity).forEach((idx) => {
      newLayout[idx] = squareState.ship_sunk;
    });
  }
...
export const placeCompShipInLayout = (ship, compLayout) => {
  ...
  entityIndices2(ship).forEach((idx) => {
    newCompLayout[idx] = squareState.ship;
  });
